...
d3.json(
    ...
    function drawArtist(artist) {
      if (artist === "0") {
        ...
        let artist_feats = tidy(
          _artist_feats,
          filter(
            (d) => !albums_meta.map((a) => a.album_id).includes(d.album_id)
          ),
        ...
        artist_feats.forEach((d) =>
          d.plays.forEach((d) => _feat_plays.push(d))
        );

        let feat_plays = tidy(
          _feat_plays,
          filter(
            (d) => !album_plays.map((a) => a.album_id).includes(d.album_id)
          ),
          mutate({
            year: (d) => d.listen_date.slice(0, 4),
            play_code: 5,
  ...
  .catch(function (error) {
    console.error("Error loading artist_level.json:", error);
  });
